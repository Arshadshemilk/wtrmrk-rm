...
"""
Orbit Wars — Commander Agent
Architecture: WorldModel + Mission-Based Planning + Predictive Physics

Strategy:
  1. Build a forward-simulated WorldModel of all planets and fleets
  2. Generate scored Missions (capture, snipe, swarm, rescue, reinforce,
     crash_exploit, gang_up, elimination)
  3. Execute missions in priority order within a time budget
  4. Evacuate doomed planets and forward rear ships to the front
  5. **Dominator Endgame**: Ship hoarding, military Keynesianism – 
     NEVER attack a planet whose capture cost exceeds its remaining production value.
"""
...
# --- DOMINATOR TIMING ---
# Play conservatively from turn 400 onward, extreme caution after 440.
LATE_REMAINING_TURNS = 100          # was 70
...
REAR_SEND_RATIO_TWO_PLAYER = 0.95     # was 0.62 – push almost everything forward
REAR_SEND_RATIO_FOUR_PLAYER = 0.95    # was 0.7
...
WEAK_ENEMY_THRESHOLD = 90
ELIMINATION_BONUS = 50.0
...
# ============================================================
# Physics (unchanged, keep as original)
# ============================================================
...
def segment_hits_sun(x1, y1, x2, y2, safety=SUN_SAFETY + 0.5):
...
# ============================================================
# World Model (mostly unchanged, minor caching adjustments not shown)
# ============================================================
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        self.is_total_war = self.remaining_steps < TOTAL_WAR_REMAINING_TURNS   # now always False
    ...
    # … all the WorldModel methods remain identical …
    ...
    def min_ships_to_own_at(self, target_id, arrival_turn, attacker_owner, planned_commitments=None,
                            extra_arrivals=(), upper_bound=None):
        return self.min_ships_to_own_by(
            target_id, arrival_turn, attacker_owner, arrival_turn=arrival_turn,
    ...
    def ships_needed_to_capture(self, target_id, arrival_turn, planned_commitments=None, extra_arrivals=()):
        return self.min_ships_to_own_at(target_id, arrival_turn, self.player,
                                         planned_commitments=planned_commitments,
                                         extra_arrivals=extra_arrivals)

# ============================================================
# Strategy functions (modified target_value & others as needed)
# ============================================================
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    """Timeline-aware adaptive margin that guarantees capture under worst-case arrivals."""
    margin = 0
    if target.owner == -1:
    ...
    # Threat scaling based on enemy ETA
    my_t, enemy_t = policy_reaction_times(target.id, policy)
...
def is_zone_secured(planet, world, policy):
    """Detects rear planets with all neighbors friendly, no near threats, and low defense needs."""
    neighbors = [p for p in world.planets if dist(planet.x, planet.y, p.x, p.y) < 28 and p.id != planet.id]
...
def consolidate_secured_fleets(world, policy, spent_total, moves, planned_commitments):
    """Pool rear ships into single fast fleets when zone is secure to maximize fleet_speed()."""
    ...
    if not frontier:
        return
    front_anchor = min(world.my_planets, key=lambda p: min(dist(p.x, p.y, f.x, f.y) for f in frontier))

    for rear in secured:
        if rear.id == front_anchor.id:
            continue
        available = max(0, int(rear.ships) - spent_total.get(rear.id, 0) - policy["reserve"].get(rear.id, 0))
        if available < 20:
            continue
