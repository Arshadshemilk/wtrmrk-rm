...
VERY_LATE_REMAINING_TURNS = 40
TOTAL_WAR_REMAINING_TURNS = 15

SAFE_NEUTRAL_MARGIN = 2
CONTESTED_NEUTRAL_MARGIN = 2
INTERCEPT_TOLERANCE = 2
...
WEAKEST_ENEMY_VALUE_MULT_4P = 2.2
WEAKEST_ENEMY_VALUE_MULT_2P = 1.25
GANG_UP_VALUE_MULT = 1.5
...
REAR_SEND_RATIO_TWO_PLAYER = 0.82
REAR_SEND_RATIO_FOUR_PLAYER = 0.88
...
# --- APEX PREDATOR ADDITIONS ---
CEASEFIRE_REMAINING_TURNS = 70
PRODUCTION_DENIAL_WEIGHT = 0.80
CONSOLIDATION_MIN_SHIPS = 20
CONSOLIDATION_MAX_TRAVEL = 30
ZONE_SECURE_RADIUS = 28
ZONE_SECURE_ENEMY_DIST = 18
COMET_PREPOSITION_LEAD = 5
COMET_INTERCEPT_VALUE_MULT = 1.3
...
class WorldModel:
    def __init__(self, player, step, planets, fleets, initial_by_id, ang_vel, comets, comet_ids):
        ...
        self.enemy_war_ships = self._compute_enemy_war_commitment()
    ...
    def _compute_enemy_war_commitment(self):
        """Track how many ships each enemy has committed to attacking other enemies."""
        war_ships = defaultdict(int)
        for target_id, arrivals in self.arrivals_by_planet.items():
            target = self.planet_by_id[target_id]
            if target.owner == -1 or target.owner == self.player:
                continue
            for _, attacker_owner, ships in arrivals:
                if attacker_owner == self.player or attacker_owner == -1:
                    continue
...
def detect_enemy_planet_battles(world):
    """Detect where an enemy fleet attacks another enemy's planet â€” creating weak spots."""
    battles = []
...
def opening_filter(target, arrival_turns, needed, src_available, world, policy):
    """Adaptive opening filter - NEVER blocks reachable high-value targets.
    Returns True to SKIP (filter out) a target, False to ALLOW it.
    """
    ...
    # NEVER filter high-production targets we can reach with advantage
    if (
    ...
    # For any target where we arrive first or equal, allow it
    if reaction_gap >= 0 and arrival_turns <= ROTATING_OPENING_MAX_TURNS:
        return False
    ...
    if world.is_four_player:
        ...
        # Allow if affordable AND we can get there in time AND we have any reaction advantage
        if (
            affordable
            ...
            return False
        # Also allow high-prod targets even if slightly contested
        if target.production >= 3 and arrival_turns <= 12 and reaction_gap >= 0:
            return False
        return True
...
def target_value(target, arrival_turns, mission, world, modes, policy):
    ...
    # === SMART CEASEFIRE WITH PRODUCTION DENIAL ===
    if world.remaining_steps < CEASEFIRE_REMAINING_TURNS and target.owner not in (-1, world.player):
        cost_to_take = max(0, int(target.ships))
        my_production_gain = target.production * turns_profit
        enemy_production_denial = target.production * turns_profit * PRODUCTION_DENIAL_WEIGHT
        true_roi = my_production_gain + enemy_production_denial - cost_to_take
        if true_roi < 0:
            return -1.0
    ...
    # Production denial bonus for enemy planets
    if target.owner not in (-1, world.player):
        value += target.production * turns_profit * PRODUCTION_DENIAL_WEIGHT
    ...
    # Weakest enemy targeting bonus (critical for 4P elimination drive)
    ...
    # === THREAT MATRIX: prioritize enemies at peace (growing quietly) ===
    if (
        target.owner not in (-1, world.player)
        and world.is_four_player
        and world.enemy_war_ships.get(target.owner, 0) < 15
    ):
        value *= 1.12

    # === ADAPTIVE 4P ROTATING NEUTRAL ===
    if world.is_four_player and target.owner == -1 and not world.is_static(target.id):
        my_t, enemy_t = policy_reaction_times(target.id, policy)
        if my_t <= enemy_t - 2:
            value *= 1.08  # We're closer - bonus
        elif my_t <= enemy_t + 1:
            value *= 0.88  # Contested - moderate discount
        else:
            value *= 0.65  # We're far behind - heavy discount
...
def preferred_send(target, base_needed, arrival_turns, src_available, world, modes, policy):
    """Threat-aware adaptive margin with production-during-travel compensation."""
    margin = 0
    if target.owner == -1:
    ...
    # Threat scaling based on enemy reaction time
    my_t, enemy_t = policy_reaction_times(target.id, policy)
    if enemy_t < arrival_turns + 5:
        margin += max(3, int(target.production * 2.5))
    ...
    if world.is_four_player:
        ...
        if modes["domination"] > 0.15:
            margin = max(0, margin - 2)
    else:
        if modes["domination"] < -0.1:
            margin += 4

    # Production during travel compensation
    margin += int(target.production * (arrival_turns / 3.0))

    # Static/long-travel/comet adjustments
    if world.is_static(target.id):
    ...
    return min(src_available, base_needed + margin)
...
def apply_score_modifiers(base_score, target, mission, world):
    ...
    if target.id in world.exposed_planet_ids:
        score *= 1.25
    # Bonus for targeting weakest enemy
    if target.owner not in (-1, world.player) and world._weakest_enemy == target.owner:
        score *= 1.15
    return score


# === FLEET CONSOLIDATION: Pool rear ships for speed advantage ===


def is_zone_secured(planet, world, policy):
    """Detect rear planets with all neighbors friendly and no near threats."""
    neighbors = [
        p for p in world.planets
        if dist(planet.x, planet.y, p.x, p.y) < ZONE_SECURE_RADIUS and p.id != planet.id
    ]
    if any(p.owner != world.player for p in neighbors):
        return False
    my_t, enemy_t = policy_reaction_times(planet.id, policy)
    if enemy_t < ZONE_SECURE_ENEMY_DIST:
        return False
    keep = world.keep_needed_map.get(planet.id, 0)
    if keep > planet.ships * 0.4:
        return False
    return True


def consolidate_secured_fleets(world, policy, spent_total, moves, planned_commitments):
    """Pool rear ships into single fast fleets for speed advantage."""
    if world.is_very_late or not world.my_planets:
        return

    secured = [p for p in world.my_planets if is_zone_secured(p, world, policy)]
    if len(secured) < 2:
        return

    frontier = world.enemy_planets or world.neutral_planets
    if not frontier:
        return
    front_anchor = min(
        world.my_planets,
        key=lambda p: min(dist(p.x, p.y, f.x, f.y) for f in frontier),
    )
    ...
    for rear in secured:
        ...
        available = max(
            0,
            int(rear.ships) - spent_total.get(rear.id, 0) - policy["reserve"].get(rear.id, 0),
        )
        if available < CONSOLIDATION_MIN_SHIPS:
            continue

        aim = world.plan_shot(rear.id, front_anchor.id, available)
        if aim is None:
            continue
        angle, turns, _, _ = aim
        if turns > CONSOLIDATION_MAX_TRAVEL:
            continue
        moves.append([rear.id, float(angle), int(available)])
        spent_total[rear.id] += available
        planned_commitments[front_anchor.id].append((turns, world.player, int(available)))
...
def build_gang_up_missions(world, policy, planned_commitments, modes):
    """Attack enemy planets weakened by inter-enemy combat (4P tactic)."""
    missions = []
    for battle in detect_enemy_planet_battles(world):
        ...
        for src in world.my_planets:
            ...
            option = ShotOption(
                ...
                mission="capture", anchor_turn=desired_arrival,
            )
            missions.append(Mission(
...
def build_comet_intercept_missions(world, policy, planned_commitments, modes):
    ...
    for comet_id in world.comet_ids:
        ...
        # Check if comet is in our quadrant (closer to us than enemies)
        ...
        if my_nearest_dist > enemy_nearest_dist + 5:
            continue  # Not our quadrant's comet

        # Find best source to intercept
        for src in world.my_planets:
            src_available = policy["attack_budget"].get(src.id, 0)
            if src_available < PARTIAL_SOURCE_MIN_SHIPS:
                continue
            ...
            seeded = world.best_probe_aim(
                ...
                hints=(int(comet.ships) + 2, int(comet.ships) + 6),
            )
            if seeded is None:
                continue
            _, rough_aim = seeded
            ...
            if rough_turns > COMET_MAX_CHASE_TURNS:
                continue
            ...
            value = comet.production * comet_life_left * COMET_INTERCEPT_VALUE_MULT
            if value <= 0:
                continue
...
def plan_moves(world, deadline=None):
    ...
    def compute_live_doomed():
        """Distance-aware evacuation: dynamic horizon based on retreat distance."""
        doomed = set()
        for planet in world.my_planets:
            ships_avail = source_inventory_left(planet.id)
            if ships_avail < DOOMED_MIN_SHIPS:
                continue

            # Calculate minimum retreat time to nearest safe ally
            safe_allies = [a for a in world.my_planets if a.id != planet.id]
            if safe_allies:
                nearest_safe_dist = min(
                    dist(planet.x, planet.y, a.x, a.y) for a in safe_allies
                )
                retreat_speed = fleet_speed(max(1, ships_avail))
                min_retreat_turns = int(math.ceil(nearest_safe_dist / retreat_speed)) + 2
            else:
                min_retreat_turns = 12

            evac_horizon = max(8, min(24, min_retreat_turns + 3))

            status = world.hold_status(
                planet.id,
                planned_commitments=planned_commitments,
                horizon=evac_horizon,
            )
            if (
                not status["holds_full"]
                and status["fall_turn"] is not None
                and status["fall_turn"] <= evac_horizon
            ):
    ...
    # === FLEET CONSOLIDATION: pool rear ships for speed ===
    consolidate_secured_fleets(world, policy, spent_total, moves, planned_commitments)
    ...
    # Comet interception missions - pre-positioning
    if allow_heavy_phase():
        missions.extend(build_comet_intercept_missions(world, policy, planned_commitments, modes))

    # Gang-up missions â€” exploit inter-enemy combat (4P advantage)
    ...
    if not world.is_very_late and allow_optional_phase():
        for src in world.my_planets:
            ...
            plan = settle_plan(
                src, target, src_left, min(src_left, send),
                world, planned_commitments, modes, policy,
                mission="capture",
            )
            if plan is None:
